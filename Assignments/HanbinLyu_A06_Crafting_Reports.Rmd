...
```{r monthly ozone concentration 2018, echo = FALSE, warning = FALSE, fig.height = 3, fig.width = 5, fig.align = "left", fig.cap = "2018 monthly Ozone levels"}
epa_data %>% 
  filter(year(Date) == 2018) %>% 
  mutate(Month = month(Date, label = TRUE)) %>% 
  ggplot(aes(x = Month, y = Ozone)) +
  geom_boxplot() +
  labs(title = "Monthly Ozone Concentration (2018)", x = "Month",
       y = "Concentration (µg/m³)") +
  theme_minimal()
```

```{r monthly ozone concentration 2019, echo = FALSE, warning = FALSE,fig.height = 3, fig.width = 5, fig.align = "left", fig.cap = "2019 monthly Ozone levels"}
epa_data %>% 
  filter(year(Date) == 2019) %>% 
  mutate(Month = month(Date, label = TRUE)) %>% 
  ggplot(aes(x = Month, y = Ozone)) +
  geom_boxplot() +
  labs(title = "Monthly Ozone Concentration (2019)", x = "Month",
       y = "Concentration (µg/m³)") +
  theme_minimal()
```
---
...
https://github.com/hanbinlyu/EDE_Fall2024/tree/main/Assignments
